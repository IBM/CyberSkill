#!/usr/bin/env bash
# db2_getDatabaseConnections.sh — POST a SQL query to the datasource API
# Example: ./db2_getDatabaseConnections
...
###############################################################################
{
  ...
  curl --silent --show-error --location --request POST \
       'http://192.168.100.15:80/api/getDatabaseConnections' \
