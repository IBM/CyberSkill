#!/usr/bin/env bash
# db2_select_from_crm.tbl_calls <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.tbl_calls db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, name, duration_hours, duration_minutes, date_start FROM crm.tbl_calls WHERE duration_hours >= {INT} FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
