#!/usr/bin/env bash
# db2_select_from_crm.tbl_marketing_template  <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.tbl_marketing_template db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, subject FROM crm.tbl_marketing_template FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
