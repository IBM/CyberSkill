#!/usr/bin/env bash
# db2_insert_into_crm.tbl_email_lists.sh <datasource> — POST a SQL query to the datasource API
# Example: ./db2_insert_into_crm.tbl_email_lists.sh db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "INSERT INTO crm.tbl_email_lists (id,email_address,email_address_caps,date_created) VALUES ('{STRING}-auto-auto-{STRING}','{STRING}@{STRING}.COM', '{STRING}@{STRING}.COM',CURRENT_TIMESTAMP);"
  }
EOF
)
