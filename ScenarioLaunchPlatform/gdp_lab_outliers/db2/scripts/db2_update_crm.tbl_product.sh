#!/usr/bin/env bash
# db2_update_crm.tbl_product.sh <datasource> — POST a SQL query to the datasource API
# Example: ./db2_update_crm.tbl_product db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "UPDATE crm.tbl_product SET quantity = quantity + 1 WHERE id = (SELECT id FROM (SELECT id FROM crm.tbl_product ORDER BY RAND() FETCH FIRST 1 ROW ONLY) AS r);"
  }
EOF
)
