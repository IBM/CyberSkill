#!/usr/bin/env bash
# db2_select_from_crm.tbl_marketing_campaign  <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.tbl_marketing_campaign  db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, email_id, template_id, campaign_date FROM crm.tbl_marketing_campaign WHERE id >= {INT} ORDER BY campaign_date DESC FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
