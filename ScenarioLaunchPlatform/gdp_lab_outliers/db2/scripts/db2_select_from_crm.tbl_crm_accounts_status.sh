#!/usr/bin/env bash
# db2_select_from_crm.tbl_crm_accounts_status <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.tbl_crm_accounts_status db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, status FROM crm.tbl_crm_accounts_status WHERE id > {INT} FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
