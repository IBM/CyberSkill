#!/usr/bin/env bash
# db2_select_from_crm.crm.tbl_bugs.sh <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.crm.tbl_bugs db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, name, bug_number, status, priority FROM crm.tbl_bugs WHERE bug_number BETWEEN {INT} AND {INT} AND status = 'Open' FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
