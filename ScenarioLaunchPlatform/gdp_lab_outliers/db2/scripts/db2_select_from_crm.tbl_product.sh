#!/usr/bin/env bash
# db2_select_from_crm.tbl_product.sh <datasource> — POST a SQL query to the datasource API
# Example: ./db2_select_from_crm.tbl_product db2_192.168.100.15_crm_polly
...
PAYLOAD=$(cat <<EOF
{
  ...
  "query_loop": 1,
  "sql": "SELECT id, name, quantity, price FROM crm.tbl_product WHERE quantity < {INT} ORDER BY quantity ASC FETCH FIRST {INT} ROWS ONLY;"
  }
EOF
)
