...
if [ "$#" -ne 3 ]; then
    ...
    echo "  - baseline_TIMESTAMP.sh (Hours 1-4: 50 alters/hour)"
    echo "  - anomaly_TIMESTAMP.sh (Hour 5: 1000 alters)"
...
UNIQUE_ID="alter_${TIMESTAMP}"

# Generate unique user and database names
DB_USER="${UNIQUE_ID}_user"
DB_USER_PASSWORD="AlterPass_${TIMESTAMP}_$(openssl rand -hex 4 2>/dev/null || echo 'default')"
...
# Purpose: Run ALTER operations for baseline creation (Hours 1-4)
# Pattern: 50 alters/hour for 4 hours
...
# Function to execute ALTER commands
execute_alters() {
    local num_alters="$1"
    local hour_num="$2"
    local batch_log="$LOG_DIR/baseline_${UNIQUE_ID}_hour_${hour_num}.log"

    log_message "Hour ${hour_num}: Executing ${num_alters} ALTER commands as user '$DB_USER'..."

    # Create 100 test tables (only once)
    for t in $(seq 1 100); do
        mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_USER_PASSWORD" "$DB_NAME" >> "$batch_log" 2>&1 <<EOF
CREATE TABLE IF NOT EXISTS alter_test_table_${t} (
...
EOF
    ...
    for i in $(seq 1 $num_alters); do
        local table_num=$(( ((i - 1) % 100) + 1 ))
        local col_name="col_h${hour_num}_${i}"
        mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_USER_PASSWORD" "$DB_NAME" >> "$batch_log" 2>&1 <<EOF
ALTER TABLE alter_test_table_${table_num} ADD COLUMN ${col_name} VARCHAR(50);
EOF
    done

    mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_USER_PASSWORD" "$DB_NAME" >> "$batch_log" 2>&1 <<EOF
SELECT CONCAT('Hour ${hour_num}: ${num_alters} ALTER commands completed') AS Result;
EOF

    if [ $? -eq 0 ]; then
        log_message "  ✓ Hour ${hour_num}: ${num_alters} alters completed successfully"
        return 0
    else
        log_message "  ✗ Hour ${hour_num}: ALTER operations failed (check $batch_log)"
        return 1
    fi
}
...
main_execution() {
    log_message "============================================================================"
    log_message "Baseline ALTER Operations (Hours 1-4)"
    log_message "============================================================================"
    ...
    log_message "Database: $DB_NAME"
    log_message "User: $DB_USER"
    log_message "Pattern: 50 alters/hour for 4 hours (BASELINE)"
    ...
    log_message ""
    log_message "============================================================================"
    log_message "Baseline ALTER Operations Complete!"
    log_message "============================================================================"
    log_message "Total Batches: 4"
    log_message "Total ALTERs: 200 (baseline)"
...
echo "============================================================================"
echo "Starting Baseline ALTER Operations in Background"
echo "============================================================================"
echo "Unique ID: $UNIQUE_ID"
echo "Database: $DB_NAME"
echo "User: $DB_USER"
...
# Purpose: Run ALTER operations for anomaly creation (Hour 5)
# Pattern: 1000 alters in hour 5 (SPIKE)
# Generated: GENERATION_TIME
# Unique ID: UNIQUE_ID_VALUE
# ============================================================================
...
execute_alters() {
    ...
    log_message "Hour 5: Executing ${num_alters} ALTER commands as user '$DB_USER' (SPIKE!)..."

    for i in $(seq 1 $num_alters); do
        local table_num=$(( ((i - 1) % 100) + 1 ))
        local col_name="col_spike_${i}"
        mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_USER_PASSWORD" "$DB_NAME" >> "$batch_log" 2>&1 <<EOF
ALTER TABLE alter_test_table_${table_num} ADD COLUMN ${col_name} VARCHAR(50);
EOF
    done
    ...
    mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_USER_PASSWORD" "$DB_NAME" >> "$batch_log" 2>&1 <<EOF
SELECT CONCAT('Hour 5: ${num_alters} ALTER commands completed (SPIKE!)') AS Result;
EOF

    if [ $? -eq 0 ]; then
        log_message "  ✓ Hour 5: ${num_alters} alters completed successfully (SPIKE!)"
        return 0
    else
        log_message "  ✗ Hour 5: ALTER operations failed (check $batch_log)"
        return 1
    fi
}
...
main_execution() {
    log_message "============================================================================"
    log_message "Anomaly ALTER Operations (Hour 5 - SPIKE)"
    log_message "============================================================================"
    log_message "Unique ID: $UNIQUE_ID"
    log_message "Host: $DB_HOST"
    log_message "Port: $DB_PORT"
    log_message "Database: $DB_NAME"
    log_message "User: $DB_USER"
    log_message "Pattern: 1000 alters in hour 5 (ANOMALY SPIKE)"
    log_message "============================================================================"
    log_message ""
    ...
    log_message ""
    log_message "============================================================================"
    log_message "Anomaly ALTER Operations Complete!"
    log_message "============================================================================"
    log_message "Total ALTERs: 1000 (SPIKE)"
    log_message "Total Duration: ${minutes}m ${seconds}s"
    log_message "============================================================================"
    log_message ""
    log_message "COMPLETE: Baseline (200 alters) + Anomaly (1000 alters) = 1200 total"
    log_message "============================================================================"
}

# Always run in background
echo "============================================================================"
echo "Starting Anomaly ALTER Operations in Background"
echo "============================================================================"
echo "Unique ID: $UNIQUE_ID"
echo "Database: $DB_NAME"
echo "User: $DB_USER"
